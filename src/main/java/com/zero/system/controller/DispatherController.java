...
import com.zero.system.model.Role;
...
//    }
    ...
    @PostMapping("/forget")
    @ResponseBody
    public AjaxResult forget(Admin admin){
        int count=0;
        Admin admin1 = adminService.selectByName(admin.getUsername());
        if(admin.getEmail().equals(admin1.getEmail())&&admin.getPhone().equals(admin1.getPhone())){
            admin1.setPassword(passwordEncoder.encode("123"));
            count = adminService.editByAdmin(admin1);
        }
        if(count >= 1){
            ajaxResult.ajaxTrue("重置密码成功 密码为123");
        }else{
            ajaxResult.ajaxFalse("系统错误");
        }
        return ajaxResult;
